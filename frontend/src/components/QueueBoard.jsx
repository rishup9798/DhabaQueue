...
function QueueRow({ position, entry, onNotify, onSeat, onRemove }) {
  ...
  const name = entry.customer?.name || formatPhone(entry.customerPhoneNumber);
  const isRegular = (entry.customer?.visitCount ?? 0) > 1;

  return (
    <div
      className={`flex items-center gap-4 px-6 py-4 ${
        entry.isAtRisk ? "bg-marigold/10 border-l-4 border-marigold" : "border-l-4 border-transparent"
      }`}
    >
      <span className="font-mono text-2xl text-paper w-10 shrink-0">#{position}</span>

      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="text-paper font-medium truncate">{name}</span>
          {isRegular && (
        ...
        <p className="text-muted text-sm">
          ...
          {entry.isAtRisk && <span className="text-marigold"> · over estimate</span>}
        </p>
      </div>
