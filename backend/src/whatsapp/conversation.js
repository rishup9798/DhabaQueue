...
export async function handleIncomingMessage({ restaurantId, phoneNumber, text }) {
  ...
  if (existingEntry) {
    if (normalized === "cancel" && existingEntry.status !== "SEATED") {
      ...
      return {
        reply: "You've been removed from the queue. Hope to see you again!",
        queueChanged: true,
      };
    }
    ...
    if (existingEntry.status === "NOTIFIED") {
      return {
        reply: "Your table is ready! Please proceed to the restaurant and check in with the staff.",
        queueChanged: false,
      };
    }
    ...
    if (existingEntry.status === "SEATED") {
      ...
      if (foodOrder) {
        return {
          reply: `You're seated. Your food order is currently ${foodOrder.status.toLowerCase()}.`,
          queueChanged: false,
        };
      }

      return {
        reply: "You're already seated. Please ask our staff if you need anything.",
        queueChanged: false,
      };
    }
    ...
    return {
      reply: `You're #${position} in line. Estimated wait: ~${existingEntry.estimatedWaitMinutes} min. Reply "status" anytime, or "cancel" to leave the queue.`,
      queueChanged: false,
    };
  }
  ...
  if (!state) {
    if (customer?.name) {
      ...
      return {
        reply: `Welcome back, ${customer.name}! How many people in your party today?`,
        queueChanged: false,
      };
    }
    ...
    if (await canUseAi(phoneNumber, restaurantId)) {
      ...
      if (name) {
        ...
        return {
          reply: `Thanks, ${name}! How many people are in your party?`,
          queueChanged: false,
        };
      }
    }
    ...
    return {
      reply: "Welcome! What name should we put your table under?",
      queueChanged: false,
    };
  }
  ...
  if (state.step === "AWAITING_NAME") {
    ...
    if (!name) {
      return {
        reply: "Just your name is fine, e.g. Ramesh",
        queueChanged: false,
      };
    }
  ...
  if (state.step === "AWAITING_PARTY_SIZE") {
    ...
    if (
      (!partySize || partySize < 1 || partySize > 30) &&
      (await canUseAi(phoneNumber, restaurantId))
    ) {
      const extracted = await extractIntakeDetails(text);
    ...
    if (!partySize || partySize < 1 || partySize > 30) {
      return {
        reply: "Please reply with just a number for your party size, e.g. 4",
        queueChanged: false,
      };
    }
  ...
  return {
    reply: 'Reply "status" to check your position, or "cancel" to leave the queue.',
    queueChanged: false,
  };
}
...
async function canUseAi(phoneNumber, restaurantId) {
  return (
    isAiEnabled() &&
    (await checkAndConsumeAiQuota(phoneNumber, restaurantId))
  );
}

function looksLikeStatusRequest(normalized) {
  return ["status", "position", "how long", "update"].some((phrase) =>
    normalized.includes(phrase)
  );
}

async function startQueueEntry({
  restaurantId,
  phoneNumber,
  partySize,
  name,
}) {
  const waitMinutes = await estimateWaitMinutes(
    restaurantId,
    partySize
  );
  ...
  return {
    reply: `You're in the queue! 🎉 You're #${position} in line for ${partySize} people. Estimated wait: ~${waitMinutes} min. We'll message you when your table is ready.`,
    queueChanged: true,
  };
}
